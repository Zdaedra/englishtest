...
- Стиль однородный, темы совпадают. Мелочь: `lead-11` («GOAL» на доске),
  `leadership-2` — текст в кадре вопреки «без надписей» (кандидаты на перегенер).
- СТАТУС: PNG готовы, НЕ сконвертированы в JPG и НЕ задеплоены — жду «ок» на
  convert→deploy (all 89). `cover_url_for` уже отдаёт v3 женским аккаунтам.
